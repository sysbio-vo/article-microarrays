...
pl1 <- scatterPlot(df1, paste(studies[1,]$ID, " versus ", studies[2,]$ID, sep=""),
                        lin, cub, c(2, 15, 2, 15), laby="log2 probeset intensity")

# hgu versus hugene, brainarray pipeline
eset1<-read.table(paste("../allsamples_exprs/", studies[1,]$ID, "_allsamples_exprs_brainarray.tsv", sep=""), header=TRUE)
eset1.tnbc <- eset1[,colnames(eset1) %in% pdata1.tnbc$SampleAccessionNumber]
...
eset3<-read.table(paste("../allsamples_exprs/", studies[3,]$ID, "_allsamples_exprs_brainarray.tsv", sep=""), header=TRUE)
...
save_plot(paste("../plots/scatterPlots/arrays_comparison.pdf", sep=""),
