...
diagPlot<-function(model){
  ...
  p5<-ggplot(model, aes(.hat, .stdresid))+geom_point(aes(size=.cooksd), na.rm=TRUE, alpha=0.3)+
  ...
  p6<-ggplot(model, aes(.hat, .cooksd))+geom_point(na.rm=TRUE, alpha=0.3)+stat_smooth(method="loess", na.rm=TRUE)+
      xlab("Leverage hii")+ylab("Cook's Distance")+
      ggtitle("Cook's dist vs Leverage hii/(1-hii)")+
      geom_abline(slope=seq(0,3,0.5), color="gray", linetype="dashed")+
      theme_bw()
  
  return(list(rvfPlot=p1, sclLocPlot=p3, rvlevPlot=p5, cvlPlot=p6))
}